...
namespace MB.Web
{
    ...
    public class SimpleHttpListener
    {
        ...
        public void Start()
        {
            ...
            {
                Console.WriteLine(
                    "ERROR! Windows XP SP2 or Server 2003 is required to use the HttpListener class."
                );
                return;
            }
            ...
            catch (HttpListenerException ex)
            {
                ...
                Console.WriteLine(
                    "make sure the user has the rights for this port or run as administrator"
                );
...
// HTTP Response Header "Content-Type" configured incorrectly on the server for file Build/Build.wasm.br , should be "application/wasm". Startup time performance will suffer.
                        ...
                        OnReceivedRequest?.Invoke(request, response);
                    }
        ...
        /// </summary>
        public Action<HttpListenerRequest, HttpListenerResponse>? OnReceivedRequest { get; set; }
